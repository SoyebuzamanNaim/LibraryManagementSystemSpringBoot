document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!bookId) {
    window.LMS.UI.showToast("Book ID not found", "error");
    setTimeout(() => (window.location.href = "/books"), 1000);
    return;
  }

  const book = window.LMS.Books.getById(bookId);
  if (!book) {
    window.LMS.UI.showToast("Book not found", "error");
    setTimeout(() => (window.location.href = "/books"), 1000);
    return;
  }
  ...
  publications.forEach((pub) => {
    ...
    if (pub.id == book.publicationId) option.selected = true;
  ...
  vendors.forEach((vendor) => {
    ...
    if (vendor.id == book.vendorId) option.selected = true;
    vendorSelect.appendChild(option);
  });

  // Populate form
  document.getElementById("title").value = book.title || "";
  document.getElementById("authors").value = Array.isArray(book.authors)
    ? book.authors.join(", ")
    : book.authors || "";
  document.getElementById("isbn").value = book.isbn || "";
  document.getElementById("totalCopies").value = book.totalCopies || 0;
  document.getElementById("availableCopies").value = book.availableCopies || 0;
  document.getElementById("categories").value = Array.isArray(book.categories)
    ? book.categories.join(", ")
    : book.categories || "";
  document.getElementById("purchaseDate").value = book.purchaseDate || "";
  ...
  document.getElementById("bookForm").addEventListener("submit", (e) => {
    ...
    // Update book
    try {
      window.LMS.Books.update(bookId, formData);
      window.LMS.UI.showToast("Book updated successfully", "success");
      setTimeout(() => {
        window.location.href = "/books";
      }, 500);
    } catch (error) {
      window.LMS.UI.showToast("Failed to update book", "error");
    }
  });
});
